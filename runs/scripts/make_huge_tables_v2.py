    ...
    def shared_step(self, x1, cond):
        ...
        if self.use_score_head:
            if self.score_loss_type == "tong":
                score_loss = F.mse_loss(sigma_t * pred_score, -eps)
            elif self.score_loss_type == "rl":
                score_loss = F.mse_loss((1 - (1 - self.sigma_min) * t) * pred_score, t * ut - xt)
            else:
...
class NODEWrapper_with_ratio_tvf(torch.nn.Module):
    ...
    def forward(self, t, x, *args, **kwargs):
        ...
        if self.model.use_score_head:
            ...
            if self.model.requires_noise:
                score_u = 2 * (t * ut - x) / self.model.sigma ** 2 / (t + 1e-2)
                score_v = 2 * (t * vt - x) / self.model.sigma ** 2 / (t + 1e-2)
            else:
...
def main(cfg: DictConfig):
    ...
    for requires_noise in noise:
        if requires_noise:
            score_loss_types = ["rnl", "tong"]
            sigmas = [1, 0.75, 0.5, 0.25]
        else:
            score_loss_types = ["rnl", "rl"]
    ...
    with open(f"/home/icb/egor.antipov/scFM_density_estimation/notebooks/tests/table_results/{cfg.run_type}_results_time_v2.pkl", "wb") as f:
